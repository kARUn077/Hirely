    ...
    <div className="w-full bg-[#f4e1d2] p-5 rounded-md shadow-lg border border-[#d3b29d]">
      ...
      {filterData.map((data, index) => (
        <div key={index} className="mt-4">
          <h1 className="font-bold text-md text-[#5a4432]">
            {data.filterType}
          </h1>
          <RadioGroup
              ...
              return (
                <div className="flex items-center space-x-2 my-2" key={itemId}>
                  <RadioGroupItem
                    value={item.value}
                    id={itemId}
                    className="text-[#6b4f32]"
                  />
                  <Label htmlFor={itemId} className="text-[#6b4f32]">
                    {item.label}
                  </Label>
                </div>
              );
      ...
      <button
        ...
        className="mt-6 text-sm bg-[#6b4f32] text-white px-3 py-1 rounded hover:bg-[#5a4432]"
      >
        Clear Filters
      </button>
    </div>
